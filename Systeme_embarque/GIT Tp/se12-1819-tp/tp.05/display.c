...
static void memory_size(uint32_t x1, uint32_t x2, uint32_t y1, uint32_t y2) {
	...
	case 0:
		...
		break;
	case 270:
		oled_memory_size(y1, y2, (SCREEN_HEIGHT - 1) - x2,
				(SCREEN_HEIGHT - 1) - x1);
		break;
	case 180:
		oled_memory_size((SCREEN_WIDTH - 1) - x2, (SCREEN_WIDTH - 1) - x1,
				(SCREEN_HEIGHT - 1) - y2, (SCREEN_HEIGHT - 1) - y1);
		break;
	case 90:
		oled_memory_size((SCREEN_WIDTH - 1) - y2, (SCREEN_WIDTH - 1) - y1, x1,
				x2);
		break;
	}
}
...
void display_clear() {
	memory_size(0, SCREEN_WIDTH - 1, 0, SCREEN_WIDTH - 1);
...
void display_rect(uint32_t x0, uint32_t y0, uint32_t w, uint32_t h,
		uint32_t color) {
