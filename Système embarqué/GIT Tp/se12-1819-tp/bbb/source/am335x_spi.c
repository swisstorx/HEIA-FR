	...
	uint32_t word_len)
{
	...
	chan->chconf =    (clkg<<29)	// clock granularity 
			| (2<<25)	// chip select time control 2.5 cycles
			| (0<<21)	// spienslv = 0
			| (0<<20)	// force = 0
			| (0<<19)	// turbo = 0
...
int am335x_spi_read(
	...
	uint32_t buffer_len)
{
	(void)cmd_word;(void)buffer;(void)buffer_len;
	volatile struct am335x_spi_ctrl* spi = spi_ctrl[ctrl];
	volatile struct am335x_spi_channel* chan = &spi->channel[channel];
	(void)chan;
	...
	uint32_t buffer_len)
{
	...
	//printf ("stat=%04x, irq=%x\n", chan->chstat, spi->irqstatus);
	...
	chan->chconf &= ~((2<<12) | (1<<27) | (1<<20)); // restore chconf
	spi->xferlevel = 0;
